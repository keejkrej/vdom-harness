...
Live closed-loop Obs cell (not #12). One official airline 0731 episode (task 44); hung-first Obs on **those** traces. Measured keyed run: episode completed (`obs.arm=I_loop`, `user_stop`); `freshHang=false`; **hole still open**. I_sku not licensed. `controllerReplay=false`. Not a score, not a hang, not a dump: `improve-live-0731-hang-obs-isku.json`.
...
Serving-step X_n dump after #18 (holes (1)+(2)): same HybridState the licensed write mutated, then ONE `runTau2Turn` appends that turn's messages to `X.H`, traces to `X.M`, and serving-step E to `X.E`. `licenseE` is an own field (hung/timeout LICENSE, not copied into H). Dump refuses if that own field is missing (does not invent `licenseE` from `X.E`). Dump serializes `X.licenseE` and `X.E` / `X.servingE` off that object — not a view overlay. `licenseE` is the hung fixture; `X.E` is serving-step E from the turn; greeting, not live hung-44 then served. Not a score, not a Pro-vs-Flash table, not stuffed H/M: `hybrid-state-serving-step-dump.json`.
